...
WITH
    ...
    both_count as (
                    SELECT
                        count(*) as num_movies
                    FROM
                        movies_netflix,movies_amazon
                    where 
                        movies_netflix.name = movies_amazon.name
                    )
SELECT both_count.num_movies/amazon_count.num_movies from amazon_count,both_count;
                   
--  O quão perto a média das notas dos filmes disponíveis na Amazon está dos filmes disponíveis na Netflix?
WITH 
ratings_per_plataform AS(
    SELECT  
        m.hosted_at as plataform ,r.rating as ratings
    FROM
        ratings as r
    INNER JOIN 
        movies as m
...
select 
    ROUND(avg_ratings_netflix.value - avg_ratings_amazon.value, 2) as avg_evaluation_diff_plataforms
from 
    avg_ratings_netflix,avg_ratings_amazon;


-- Qual ano de lançamento possui mais filmes na Netflix?

SELECT 
    LAUNCH_YEAR AS YEAR_WITH_MOST_LAUNCHES
FROM 
    MOVIES 
GROUP BY 
    LAUNCH_YEAR
ORDER BY 
    COUNT(LAUNCH_YEAR) DESC
LIMIT 1;

-- Quais filmes que não estão disponíveis no catálogo da Amazon foram *melhor avaliados?
SELECT 
        DISTINCT m.name as movie_name,
        r.rating as rating
    from movies as m
    INNER JOIN 
        ratings as r
    ON
        m.id = r.movie_id
    WHERE 
        m.name not in ( select name from movies where hosted_at = 'AMAZON') AND
        r.rating IN(4,5);
...
SELECT 
    ...
    ON
        m.id = r.movie_id
    WHERE 
        m.name not in ( select name from movies where hosted_at = 'NETFLIX') AND
        r.rating IN(4,5);

-- Quais os 10 filmes que possuem mais avaliações nas plataformas?
SELECT 
    m.name,
    count(r.movie_id) as movie_evaluations
FROM 
    ratings AS r
INNER JOIN
    movies AS m
...
SELECT 
    client_id,
    COUNT(client_id) as evaluations
FROM
    ratings
WHERE
    client_id like('AMA%') -- clients from amazon has id starting with 'AMA'
GROUP BY
    client_id
ORDER BY 
...
select
    COUNT( DISTINCT movie_id) as evaluted_products_top_evaluators
FROM 
    ratings
WHERE
    client_id IN(
...
FROM
    ratings
WHERE
    client_id like('NET%') -- clients from netflix has id starting with 'NET'
GROUP BY
    client_id
ORDER BY 
    COUNT(client_id) DESC
...
SELECT
    COUNT(movie_id) as most_recent_evaluations
FROM
    ratings
where
    rating_date  = ( 
...
-- Quais os 10 filmes mais bem avaliados na data mais recente da amzon?

SELECT
    m.name AS top_10_rating
